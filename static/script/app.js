...
$(document).on("click", "#check", function() {
    ...
    // console.log([row1, row2, row3, row4, row5, row6, row7, row8, row9])
    req = $.ajax({
        type: 'POST',
        url: '/check-solution',
        data: {
            grid: JSON.stringify([row1, row2, row3, row4, row5, row6, row7, row8, row9])
        }
    });
    ...
    req.done(function(data) {
        ...
        $("#loader").css({ "display": "none" })
        let element = $("#check-sol")
        element.css({ "display": "block" })
        element.html(data["res"])
    });
});
